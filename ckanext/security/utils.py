...
# (canada fork only): frameworked helpers
...
def _setup_totp_template_variables(context, data_dict):
    ...
    if totp_challenger is not None:
        # (canada fork only): enable TOTP
        tc['totp_configured'] = True
...
def login():
    ...
    try:
        ...
        # (canada fork only): do not create new TOTP in AJAX
        ...
        if not mfaConfigured and totp_challenger:
        ...
        if identity['mfa'] and totp_challenger:
...
def configure_mfa(id=None):
    ...
    if request.method == 'POST':
        # (canada fork only): frameworked helpers
        # (canada fork only): 2.10 support
        if tc.get('mfa_test_valid'):
            tk.h.flash_success(_('''That's a valid code. Your authenticator
                app is correctly configured for future use.'''))
        if tc.get('mfa_test_invalid'):
            tk.h.flash_error(_('''That's an incorrect code. Try scanning
...
def new(id=None):
    ...
    # (canada fork only): frameworked helpers
    tk.h.flash_success(_('''Successfully updated two factor
...
def disable(id=None):
    """SDelete a user's new security TOTP credentials"""
    gc = _get_global()
    context = {
        'model': model, 'session': model.Session,
        'user': gc.user, 'auth_user_obj': gc.userobj
    ...
    # pylons includes the rest of the url in the param,
    # so we need to strip the /new suffix
    user_id = id.replace('/disable', '')

    data_dict = {'id': user_id, 'user_obj': gc.userobj}
    user_dict = _fetch_user_or_fail(context, data_dict)
    SecurityTOTP.delete_for_user(user_dict['name'])
    log.info("Deleted the 2fa secret for user {}".format(user_id))
    tk.h.flash_success(_('''Successfully disabled two factor
            authentication. Make sure you delete the setup from
            your authenticator app.'''))
